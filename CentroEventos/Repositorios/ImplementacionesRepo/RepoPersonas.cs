...
public class RepoPersonas : IRepositorioPersona
{
    ...
    public void registrarPersona(Persona p)
    {
        ...
        if (persona == null)
        {
            ...
            var pers = _context.Personas
            .Include(p => p._permisos)
            .FirstOrDefault(pe => pe._mail == p._mail);
            if ((pers != null) && (pers._id == 1))
            {
                List<Permiso> permisos = _context.Permisos.ToList();
                foreach (Permiso permiso in permisos)
                {

                    if (!pers._permisos.Any(p => p._id == permiso._id)) // aca si el id es igual a 1, se le agregan todos los permisos, si es distinto no hace nada
                    {
                        pers._permisos.Add(permiso);
                        _context.SaveChanges();
                    }
